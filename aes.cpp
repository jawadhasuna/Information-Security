...
void ShiftRows() {
    ...
    byte tmp[2] = { state[2][0], state[2][1] };
    state[2][0] = state[2][2]; state[2][1] = state[2][3];
    state[2][2] = tmp[0]; state[2][3] = tmp[1];
...
void MixColumns() {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        Tm = state[0][i] ^ state[1][i]; Tm = (Tm << 1) ^ ((Tm >> 7) * 0x1B); state[0][i] ^= Tm ^ Tmp;
        Tm = state[1][i] ^ state[2][i]; Tm = (Tm << 1) ^ ((Tm >> 7) * 0x1B); state[1][i] ^= Tm ^ Tmp;
        Tm = state[2][i] ^ state[3][i]; Tm = (Tm << 1) ^ ((Tm >> 7) * 0x1B); state[2][i] ^= Tm ^ Tmp;
        Tm = state[3][i] ^ t;           Tm = (Tm << 1) ^ ((Tm >> 7) * 0x1B); state[3][i] ^= Tm ^ Tmp;
    }
}
